...
{
	VA_ARGS_VGET(args, const host_t*, src, const host_t*, dst,
...
CALLBACK(sad_entry_match_dst, bool,
	sad_entry_t * const entry, va_list args)
{
	VA_ARGS_VGET(args, const uint32_t*, reqid, const uint32_t*, spi,
				 const uint8_t*, proto);
...
CALLBACK(sad_entry_match_esa_id, bool,
	sad_entry_t * const entry, va_list args)
{
	VA_ARGS_VGET(args, const esa_id_type*, esa_id);
...
CALLBACK(sad_entry_match_other_esa, bool,
	sad_entry_t * const entry, va_list args)
{
	VA_ARGS_VGET(args, const esa_id_type*, esa_id, const uint32_t*, reqid);
	return entry->reqid  == *reqid &&
		   entry->esa_id != *esa_id;
}
...
{
	VA_ARGS_VGET(args, sad_entry_t*, right);
